...
d.innerHTML=_12;},loadStyle:function(_14){this.vj$.CssLoader.loadText(_14);},loadPromo:function(id,_16,_17,_18){var t=this,hid,ad=t.ads[id];if(_18){hid=t.json.merchPrefix+id+_16;}else{hid=ad.htmlID;}
var e=t.E.get(hid);if(ad.loaded){return true;}
...
var e=t.E.get(ad.htmlID),u=ad.dblclkURL;if(!u||ad.loaded){return;}
if(e&&!e.length){ad.loaded=true;if(ad.content){u+=ad.content+";";}
u+="ord="+(new Date()).getTime();e.innerHTML=t.createIFrame(ad.id,u,ad.width,ad.height);e.style.display="block";}},noAd:function(ad){var t=this,e=t.E.get(ad.htmlID),u=ad.defaultURL;if(ad.loaded){return;}
if(e&&!e.length){ad.loaded=true;if(!u||u=="collapse"||u==""){e.style.height="0px";e.style.display="none";}else{e.innerHTML=t.createIFrame(ad.id,u,ad.width,ad.height);}}},htmlAd:function(ad){var t=this,w=ad.width,h=ad.height,ex=ad.expand,ifr;w=t.adjustDim(w);h=t.adjustDim(h);var e=t.E.get(ad.htmlID);if(ad.loaded){return;}
if(e&&!e.length){ad.loaded=true;s=e.style;t.processDim(s,ex,w,h);e.innerHTML=ad.content;if(ex){ifr=t.E.get("rtm_iframe_"+ad.id);if(ifr){if(w=="auto"){w=ifr.width;s.width=w+"px";}
...
if(w!="auto"&&h!="auto"){if(ex){s.textAlign="left";}else{s.overflow="hidden";}}},executeScripts:function(_31){var m,re=/<script\b[\s\S]*?>([\s\S]*?)<\/script>/ig;while(m=re.exec(_31)){this.vj$.JsLoader.loadText(m[1]);}},formAd:function(ad){var t=this,w=ad.width,h=ad.height,f;w=t.adjustDim(w);h=t.adjustDim(h);var e=t.E.get(ad.htmlID);if(ad.loaded){return;}
if(e&&!e.length){ad.loaded=true;s=e.style;t.processDim(s,false,w,h);e.innerHTML=t.createIFrame(ad.id,"","100%","");f=t.E.get("ifrm_"+ad.id);s.display="block";f.doc=null;if(f.contentDocument){f.doc=f.contentDocument;}else{if(f.contentWindow){f.doc=f.contentWindow.document;}else{if(f.document){f.doc=f.document;}}}
if(f.doc==null){throw"RTM002";}
f.doc.open();try{f.doc.write(ad.content);}
finally{f.doc.close();}}},createIFrame:function(id,u,w,h){return"<iframe frameborder=\"no\" border=\"0\" marginwidth=\"0\" marginheight=\"0\" scrolling=\"no\""+" id=\"ifrm_"+id+"\" name=\"ifrm_"+id+"\" src=../../www/bilder/Filer_for_eBayISAPI.dll//""+u+"/""+" width=\""+w+"\" height=\""+h+"\"></iframe>";},processGlobalNavPids:function(_3b){var s="",i,a,n;if(typeof(_oGlobalNavRTMInfo)!="undefined"){a=_oGlobalNavRTMInfo.aRTMPlacementData;n=a.length;for(i=0;i<n;i++){if(s){s+=":";}
if(_3b){s+=a[i].pid;}else{s+="0";}}
if(_3b){this.mergeAds(a);}}
return s;},setTimeOut:function(){var t=this;if(!t.bRespRet){var _3e=function(){t.processTimeOut();};t.oTimeoutId=window.setTimeout(_3e,t.iTIMEOUT);}},processTimeOut:function(){var t=this,id,ad,b=t.json;t.bTimedOut=true;for(id in t.ads){ad=t.ads[id];var e=t.E.get(ad.htmlID),du=ad.defaultURL;if(ad.loaded||(e&&e.innerHTML.length)){continue;}
if(e&&!e.length){ad.loaded=true;if(!du||du=="collapse"||du==""){e.style.height="0px";e.style.display="none";}else{e.innerHTML=t.createIFrame(ad.id,du,ad.width,ad.height);}}}
...
t.bodyLoadedTime=(new Date()).getTime();},loadAd:function(ad){var t=this;if(ad.type=="doubleclick"){t.dcAd(ad);}else{if(ad.type=="html"){t.htmlAd(ad);}else{if(ad.type=="popUnder"){t.popUnderAd(ad);}else{if(ad.type=="promo"){t.hasPromo=true;t.promoAd(ad);}else{if(ad.type=="htmlform"){t.formAd(ad);}else{var b=t.json,s=b.onScrollPids,i;if(s){for(i=0;i<s.length;i++){if(ad.id==s[i]){return;}}}
t.noAd(ad);}}}}}},renderPromos:function(_6a){},getPids:function(_6b){var p="",l=_6b.length;for(i=0;i<l;i++){p+=_6b[i];if(i!=(l-1)){p+=":";}}
...
/*eof:z/uz/3d2nn02w5q513lvn21bocmpte.js*/